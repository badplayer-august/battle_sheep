...
def InitPos(mapStat):
    ...
    action = MCTS(MCTS_NODE(BattleSheepState(mapStat, sheepStat, playerID))).best_action(c_param=c_param, h=h)
...
def GetStep(playerID, mapStat, sheepStat):
    action = MCTS(MCTS_NODE(BattleSheepState(np.array(mapStat, dtype='int32').T, list(np.array(sheepStat, dtype='int32').T), playerID))).best_action(c_param=c_param, h=h)
