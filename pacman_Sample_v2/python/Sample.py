...
def getStep(playerStat, ghostStat, propsStat,otherPlayerStat,right_map,left_map,up_map,down_map):
    ...
    x = int(playerStat[0])
    y = int(playerStat[1])
    limit_x = max(int(playerStat[0]/25)+12,16)# define search space
    limit_y = max(int(playerStat[1]/25)+12,16)
    ...
    for dir in dirs:
        ...
        search(x,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map)
        result = sum(score)
        #print(result)
...
def search(x,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map): # DFS
    visit[x][y]=0
    score.append(reward_map[x][y])
    if reward_map[x][y]==-100:
        enemy=True
    if x+1<16 and x+1>=0 and y<16 and y>=0 and x+1<=limit_x and y<=limit_y and visit[x+1][y]==1 and down_map[x+1][y]==1:
        search(x+1,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map)
    else:
        return
    if x-1<16 and x-1>=0 and y<16 and y>=0 and x-1<=limit_x and y<=limit_y and visit[x-1][y]==1 and up_map[x-1][y]==1:
        search(x-1,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map)
    else:
        return
    if x<16 and x>=0 and y+1<16 and y+1>=0 and x<=limit_x and y+1<=limit_y and visit[x][y+1]==1 and right_map[x][y+1]==1:
        search(y+1,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map)
    else:
        return
    if x<16 and x>=0 and y-1<16 and y-1>=0 and x<=limit_x and y-1<=limit_y and visit[x][y-1]==1 and left_map[x][y-1]==1:
        search(y-1,y,limit_x,limit_y,visit,score,reward_map,left_map,right_map,up_map,down_map)
    else:
        return
